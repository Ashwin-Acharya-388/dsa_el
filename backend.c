...
// Build a string representation of the tree structure (for API visualization)
void get_tree_string(KDNode *node, char *buffer, int *pos, int max_len, int level) {
    if (node == NULL || *pos >= max_len - 100) return;
    
    // Add indentation
    for (int i = 0; i < level; i++) {
        if (*pos < max_len - 3) {
            strncat(buffer, "  ", max_len - *pos - 1);
            *pos += 2;
        }
    }
    ...
    // Format node info
    char line[256];
    int line_len = snprintf(line, sizeof(line), "├── %s (%c) [%.3f, %.3f]\n", 
                           node->venue.name, 
                           node->venue.type[0],
                           node->venue.x,
                           node->venue.y);
    ...
    if (*pos + line_len < max_len - 1) {
        strncat(buffer, line, max_len - *pos - 1);
        *pos += line_len;
    }
    
    get_tree_string(node->left, buffer, pos, max_len, level + 1);
    get_tree_string(node->right, buffer, pos, max_len, level + 1);
}
...
// File persistence
const char *DB_FILE = "venues.csv";

void save_venues() {
    FILE *f = fopen(DB_FILE, "w");
    if (!f) {
        printf("Error: Could not save venues to file.\n");
        return;
    }
    
    // Header
    fprintf(f, "id,name,type,x,y,color,price\n");
    
    for (int i = 0; i < venue_count; i++) {
        fprintf(f, "%d,%s,%s,%.6f,%.6f,%s,%s\n",
                venues[i].id,
                venues[i].name,
                venues[i].type,
                venues[i].x,
...
int load_venues() {
    ...
    venue_count = 0;
    while (fgets(line, sizeof(line), f) && venue_count < MAX_VENUES) {
        Venue v;
        // Parse CSV logic (simple version)
        // Format: id,name,type,x,y,color,price
        char *token;
        char *rest = line;
        
        // ID
        token = strtok(rest, ","); if(!token) continue; v.id = atoi(token);
        // Name
        token = strtok(NULL, ","); if(!token) continue; strncpy(v.name, token, MAX_NAME_LENGTH-1);
        // Type
        token = strtok(NULL, ","); if(!token) continue; strncpy(v.type, token, MAX_TYPE_LENGTH-1);
        // X
        token = strtok(NULL, ","); if(!token) continue; v.x = atof(token);
        // Y
        token = strtok(NULL, ","); if(!token) continue; v.y = atof(token);
        // Color
        token = strtok(NULL, ","); if(!token) continue; strncpy(v.color, token, 19);
        // Price (remove newline if present)
        token = strtok(NULL, ","); 
        if(token) {
            strncpy(v.price, token, 14);
            // Trim newline
            size_t len = strlen(v.price);
            if (len > 0 && v.price[len-1] == '\n') v.price[len-1] = '\0';
        } else {
            strcpy(v.price, "");
        }
    ...
    fclose(f);
    printf("Loaded %d venues from %s\n", venue_count, DB_FILE);
    return venue_count;
}

// Add new venue and rebuild tree
int add_venue_backend(const char* name, const char* type, float x, float y) {
    if (venue_count >= MAX_VENUES) return 0;
    
    Venue v;
    v.id = (int)time(NULL) + venue_count; // Simple unique ID
    strncpy(v.name, name, MAX_NAME_LENGTH - 1);
    v.name[MAX_NAME_LENGTH - 1] = '\0';
    strncpy(v.type, type, MAX_TYPE_LENGTH - 1);
    v.type[MAX_TYPE_LENGTH - 1] = '\0';
    v.x = x;
    v.y = y;
    
    // Set color based on type (simple mapping)
    if (strcmp(type, "park") == 0) strcpy(v.color, "#27ae60");
    else if (strcmp(type, "mall") == 0) strcpy(v.color, "#3498db");
    else if (strcmp(type, "cinema") == 0) strcpy(v.color, "#9b59b6");
    else if (strcmp(type, "restaurant") == 0) strcpy(v.color, "#f39c12");
    else if (strcmp(type, "metro") == 0) strcpy(v.color, "#34495e");
    else strcpy(v.color, "#ffffff");
    
    strcpy(v.price, ""); // Default empty price
...
void initialize_sample_venues() {
    ...
    Venue sample_venues[100] = {
        ...
        {1,"Cubbon Park East","park",12.9769,77.6006,"#27ae60",""},
        {2,"Cubbon Park West","park",12.9745,77.5937,"#27ae60",""},
        {3,"Lalbagh West Gate","park",12.9500,77.5800,"#27ae60",""},
        {4,"Lalbagh Lake","park",12.9485,77.5842,"#27ae60",""},
        {5,"Ulsoor Lake East","park",12.9824,77.6108,"#27ae60",""},
        {6,"Ulsoor Lake West","park",12.9792,77.6044,"#27ae60",""},
        {7,"Sankey Tank Viewpoint","park",12.9910,77.5638,"#27ae60",""},
        {8,"JP Park Amphitheatre","park",13.0328,77.5631,"#27ae60",""},
        {9,"Agara Lake Walkway","park",12.9189,77.6418,"#27ae60",""},
        {10,"Kaikondrahalli Lake Park","park",12.9108,77.6632,"#27ae60",""},
        
        {11,"Commercial Street","mall",12.9754,77.6080,"#3498db",""},
        {12,"Brigade Road","mall",12.9741,77.6072,"#3498db",""},
        {13,"Church Street","mall",12.9757,77.6051,"#3498db",""},
        {14,"Forum Rex Walk","mall",12.9729,77.6133,"#3498db",""},
        {15,"Garuda Mall Complex","mall",12.9715,77.6085,"#3498db",""},

        {16,"Ranga Shankara Theatre","cinema",12.9221,77.5903,"#9b59b6",""},
        {17,"Jagriti Theatre","cinema",12.9947,77.7571,"#9b59b6",""},
        {18,"Chowdiah Memorial Hall","cinema",13.0066,77.5795,"#9b59b6",""},
        {19,"Alliance Francaise","cinema",12.9759,77.5999,"#9b59b6",""},
        {20,"Freedom Park Amphitheatre","cinema",12.9837,77.5854,"#9b59b6",""},
        ...
        // ---------------- RESTAURANTS (51–65) ----------------
        {51,"Toit Indiranagar","restaurant",12.9719,77.6412,"#f39c12","premium"},
        {52,"Vidyarthi Bhavan","restaurant",12.9426,77.5669,"#f39c12","budget"},
        {53,"MTR Lalbagh","restaurant",12.9591,77.5857,"#f39c12","mid"},
        {54,"Koshy's","restaurant",12.9716,77.5946,"#f39c12","mid"},
        {55,"The Only Place","restaurant",12.9733,77.6117,"#f39c12","mid"},
        {56,"CTR Malleshwaram","restaurant",13.0037,77.5643,"#f39c12","budget"},
        {57,"Empire Restaurant","restaurant",12.9756,77.6044,"#f39c12","mid"},
        {58,"Truffles Koramangala","restaurant",12.9352,77.6143,"#f39c12","mid"},
        {59,"Nagarjuna Residency Road","restaurant",12.9723,77.5991,"#f39c12","mid"},
        {60,"Meghana Foods","restaurant",12.9355,77.6148,"#f39c12","mid"},
        {61,"Byg Brewski","restaurant",12.9955,77.7056,"#f39c12","premium"},
        {62,"AB's Barbecue","restaurant",12.9701,77.6370,"#f39c12","premium"},
        {63,"Ironhill Brewery","restaurant",12.9111,77.6419,"#f39c12","premium"},
        {64,"Windmills Craftworks","restaurant",12.9726,77.7294,"#f39c12","premium"},
        {65,"Rameshwaram Cafe","restaurant",12.9704,77.5940,"#f39c12","budget"},
